...
double& Poly::operator[](int power) {
    ...
    int newSize = power+1;
    double *newCoef = new double[power+1];
...
ostream& operator<<(ostream& os, const Poly& p){ //vyvod
    bool first = false; //znak
    os << "y = ";
    for (int i = p.size-1; i >= 0; --i) { //stepen
    ...
    Poly Poly::operator+(const Poly& other) const { // primer na primer
    Poly result;
    ...
    Poly Poly::operator+(double value) const { // result
    Poly result = *this;
    result[0] += value;
    return result;
}
Poly operator+(double value, const Poly& p) { //chislo na primer
    return p + value;
}
Poly Poly::operator-(const Poly& other) const { // primer na primer
    Poly result;
...
Poly Poly::operator-(double value) const { // resultat
    Poly result = *this;
    result[0] -= value;
    return result;
}
